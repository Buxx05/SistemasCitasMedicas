...
public class EspecialidadDAO {
    ...
    public List<Especialidad> listarEspecialidades() {
        ...
        String sql = "SELECT id_especialidad, nombre, descripcion FROM Especialidades ORDER BY nombre";
        ...
        try (Connection conn = ConexionDB.getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            ...
            while (rs.next()) {
                ...
                esp.setCodigo(GeneradorCodigos.generarCodigoEspecialidad(id));
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (affectedRows > 0) {
                ...
                if (generatedKeys.next()) {
                    ...
                    // ✅ Generar código en memoria (no se guarda en BD)
                    String codigo = GeneradorCodigos.generarCodigoEspecialidad(idGenerado);
    ...
    public List<Especialidad> listarEspecialidadesActivas() {
        List<Especialidad> especialidades = new ArrayList<>();
        String sql = "SELECT DISTINCT e.id_especialidad, e.nombre, e.descripcion "
